...
# 4. Package 3: pdfcraft Standalone Tarball
mkdir -p "${BUILD_DIR}/individual/pdfcraft-${VERSION}"
cp pdfcraft README.md LICENSE PDFCRAFT_DOCUMENTATION.md "${BUILD_DIR}/individual/pdfcraft-${VERSION}/"
cd "${BUILD_DIR}/individual"
tar -czvf "pdfcraft-${VERSION}-linux-x86_64.tar.gz" "pdfcraft-${VERSION}"
cd ../..

# 5. Package 4: lazy_vault Standalone Tarball & RPM
...
# 6. Full Suite Tarball
mkdir -p "${BUILD_DIR}/lazy-tools-${VERSION}"
cp play_140p audioconvert pdfcraft install.sh README.md LICENSE "${BUILD_DIR}/lazy-tools-${VERSION}/"
if [ -f "projects/lazy_vault/lazy_vault" ]; then
